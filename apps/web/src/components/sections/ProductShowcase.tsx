...
function PublishPanel({ tokens, scenario }: { tokens: Tokens; scenario: ScenarioId }) {
  ...
  const queue = initialQueue.map((item) =>
    approvedLabels.includes(item.label) ? { ...item, status: "Scheduled" } : item,
  );

  const approve = (label: string) => {
    setApprovedLabels((labels) => (labels.includes(label) ? labels : [...labels, label]));
  };
